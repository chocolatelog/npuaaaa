...
更新时间：2026-09-26。本次按明确要求追加第三问技术路线与实验记录，连同复现所需方案、JSON/JSONL、指纹和汇总归档，沿用 H 授权归档例外。不含环境、缓存、密钥、数据库及官方完整 trace。

## 当前正式成果

- [当前技术路线 H-v3.1](../技术路线_H分支_v3.1.md)
- [第三问 C400 实验记录](../实验记录_H分支_第三问C400全量_v1.0.md)
- [C400 原始日志](solve_log_problem3_full400.jsonl)、[汇总](solve_log_problem3_full400.summary.json)、[启动指纹](solve_log_problem3_full400.manifest.json)、[400 份 C 方案](plans_problem3_full400/)
- [早期五例探针](problem3_probe/summary.json)、[两例探针](problem3_probe_iter1/summary.json)：未改善且包含跳过评估，不代表全量结果。

C400 每核 100 案例，5 核平均加速比 4.468436×、逐例字节命中率平均 23.1639%。本轮保底指 B 方案在 C 下重新评估，不是历史 B 得分；旧 manifest 的 scope/target_scope 文本遗留已在实验记录中说明，原始证据不改写。
